<div class="{cycle values='container-1,container-2'}">
	...
	<div class="containerContent">
		<h3>{lang}wiki.index.ircuseronline{/lang}</h3> 
		{foreach from=$datas key=keys item=data}
			<p class="smallFont"> {$data.channelname} ({$data.usercount} {lang}wiki.index.ircuseronline.channeluser{/lang}):
		...
		{/foreach}
		<p class="smallFont">{lang}wiki.index.ircuseronline.legende{/lang}: <span style="color:{IRCUSERONLINE_COLOR_OP}">{lang}wiki.index.ircuseronline.op{/lang}</span>, <span style="color:{IRCUSERONLINE_COLOR_VOICE}">{lang}wiki.index.ircuseronline.voice{/lang}</span>
	</div>
</div>